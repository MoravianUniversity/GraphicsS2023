...
function initProgram() {
    ...
    let frag_shader = compileShader(gl, gl.FRAGMENT_SHADER,
        ...
        void main() {
            ...
            // Object color combined from texture
...
function initTextures() {
    // Load checkerboard and WebGL.png textures
    obj.push(loadTexture(createCheckerboardImage(32, 4), 0));
    obj.push(loadTexture(createCheckerboardImage(32, 32), 1));

    let image = new Image();
    image.src = 'WebGL.png';
    image.addEventListener('load', () => {
        obj.push(loadTexture(image, 2));

        // Set parameters from HTML (normally not needed)
        setParamForAllTextures(gl.TEXTURE_MIN_FILTER, document.getElementById('min').value);
        setParamForAllTextures(gl.TEXTURE_MAG_FILTER, document.getElementById('mag').value);
        setParamForAllTextures(gl.TEXTURE_WRAP_S, document.getElementById('wrap-s').value);
        setParamForAllTextures(gl.TEXTURE_WRAP_T, document.getElementById('wrap-t').value);
...
function initEvents() {
    ...
    document.getElementById('mag').addEventListener('input', () => {
        setParamForAllTextures(gl.TEXTURE_MAG_FILTER, document.getElementById('mag').value);
    });
    document.getElementById('wrap-s').addEventListener('input', () => {
        setParamForAllTextures(gl.TEXTURE_WRAP_S, document.getElementById('wrap-s').value);
    });
    document.getElementById('wrap-t').addEventListener('input', () => {
        setParamForAllTextures(gl.TEXTURE_WRAP_T, document.getElementById('wrap-t').value);
    });
}

/**
 * Sets a texture parameter for both textures.
 */
function setParamForAllTextures(param, gl_const_name) {
    let [vao, count, mode, texture1, texture2] = obj;
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, texture1);
    gl.texParameteri(gl.TEXTURE_2D, param, gl[gl_const_name]);
