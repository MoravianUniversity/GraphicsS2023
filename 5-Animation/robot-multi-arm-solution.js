...
const ARM2_ANGLE = 0; // rotates around x
...
function initScene() {
    ...
    let [base_start, base_count] = cylinder(vertices, normals, indices, BASE_HEIGHT, 0.2);
    let [lower_start, lower_count] = cube(vertices, normals, indices, [0.02, ARM1_LENGTH, 0.02]);
    let [upper_start, upper_count] = cube(vertices, normals, indices, [0.02, ARM2_LENGTH, 0.02]);
    ...
    // Upper robot arms - there are 4 of them, all copies of each other
    let upper_1_1 = createNode({
    ...
    setupListener(upper_1_1, 'upper-1.1-angle', ARM2_ANGLE);
    let upper_1_2 = copyNode(upper_1_1, {'color': [0, 0, 0.8, 1]});
    setupListener(upper_1_2, 'upper-1.2-angle', ARM2_ANGLE);
    let upper_2_1 = copyNode(upper_1_1, {'color': [0, 0, 0.6, 1]});
    setupListener(upper_2_1, 'upper-2.1-angle', ARM2_ANGLE);
    let upper_2_2 = copyNode(upper_1_1, {'color': [0, 0, 0.4, 1]});
    setupListener(upper_2_2, 'upper-2.2-angle', ARM2_ANGLE);

    // Lower robot arms - there are 2 of them, copies but with different children
    let lower_1 = createNode({
        'position': [0, BASE_HEIGHT/2 + ARM1_LENGTH/2, 0],
        'origin': [0, -ARM1_LENGTH/2, 0],
        'color': [0, 1, 0, 1],
        'children': [upper_1_1, upper_1_2],
        'start': lower_start,
        'count': lower_count,
    });
    setupListener(lower_1, 'lower-1-angle', ARM1_ANGLE);
    let lower_2 = copyNode(lower_1, {
        'color': [0, 0.8, 0, 1],
        'children': [upper_2_1, upper_2_2]
    });
    setupListener(lower_2, 'lower-2-angle', ARM1_ANGLE);

    // Robot Arm base - only one, the root of our scene
    let base = createNode({
        'position': [0, -BASE_HEIGHT/2, 0],
        'color': [1, 0, 0, 1],
        'children': [lower_1, lower_2],
        'start': base_start,
        'count': base_count,
    });
    setupListener(base, 'base-angle', BASE_ANGLE);
...
function renderNode(node, mv) {
    ...
    for (let child of node.children) { renderNode(child, mv); }
}
