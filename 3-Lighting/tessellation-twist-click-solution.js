// Renders a triangle tessellation with a twist transformation that increases with clicking (solution)
'use strict';
...
window.addEventListener('load', function init() {
    ...
    gl.uniform1f(gl.program.uTheta, 0);
...
function initProgram() {
    ...
    let vert_shader = compileShader(gl, gl.VERTEX_SHADER,
        ...
        void main() {
            ...
            gl_Position.x = x*cos(d*uTheta)-y*sin(d*uTheta);
            gl_Position.y = x*sin(d*uTheta)+y*cos(d*uTheta);
            gl_Position.zw = vec2(0.0, 1.0); // SWIZZLE!
            vColor = aColor;
        }`
    );
    // Fragment Shader
    ...
    program.uTheta = gl.getUniformLocation(program, 'uTheta'); // get the uniform location for "uTheta"
...
function onClick(e) {
    ...
    gl.uniform1f(gl.program.uTheta, theta);
    render();
}
